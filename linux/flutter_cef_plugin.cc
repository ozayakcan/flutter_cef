#include "include/flutter_cef/flutter_cef_plugin.h"
...
#include "flutter_cef_keyevent.h"
#include "flutter_cef_texture.h"

#define WEBVIEW_CEF_PLUGIN(obj)                                     \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), flutter_cef_plugin_get_type(), \
                              FlutterCefPlugin))

struct _FlutterCefPlugin
{
  GObject parent_instance;
  std::shared_ptr<flutter_cef::WebviewPlugin> m_plugin;
  FlTextureRegistrar *m_textureRegister;
  int64_t m_window;
};

G_DEFINE_TYPE(FlutterCefPlugin, flutter_cef_plugin, g_object_get_type())

std::unordered_map<int64_t, std::shared_ptr<flutter_cef::WebviewPlugin>> webviewPlugins;
...
public:
  WebviewTextureRenderer(FlTextureRegistrar *texture_register)
  {
    ...
    texture = flutter_cef_texture_new();
  ...
  virtual void onFrame(const void *buffer, int32_t width, int32_t height) override
  {
    ...
    flutter_cef::SwapBufferFromBgraToRgba((void *)texture->buffer, buffer, width, height);
...
static void flutter_cef_plugin_handle_method_call(
    FlutterCefPlugin *self,
...
static void flutter_cef_plugin_dispose(GObject *object)
{
  ...
  G_OBJECT_CLASS(flutter_cef_plugin_parent_class)->dispose(object);
}

static void flutter_cef_plugin_class_init(FlutterCefPluginClass *klass)
{
  G_OBJECT_CLASS(klass)->dispose = flutter_cef_plugin_dispose;
}

static void flutter_cef_plugin_init(FlutterCefPlugin *self) {
  self->m_plugin = std::make_shared<flutter_cef::WebviewPlugin>();
}

static void method_call_cb(FlMethodChannel *channel, FlMethodCall *method_call,
                           gpointer user_data)
{
  FlutterCefPlugin *plugin = WEBVIEW_CEF_PLUGIN(user_data);
  flutter_cef_plugin_handle_method_call(plugin, method_call);
}

void flutter_cef_plugin_register_with_registrar(FlPluginRegistrar *registrar)
{
  FlutterCefPlugin *plugin = WEBVIEW_CEF_PLUGIN(
      g_object_new(flutter_cef_plugin_get_type(), nullptr));
...
FLUTTER_PLUGIN_EXPORT int initCEFProcesses(int argc, char **argv)
{
  ...
  return flutter_cef::initCEFProcesses(main_args);
}
