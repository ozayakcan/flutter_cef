...
# if '#include <flutter_cef/flutter_cef_plugin.h>' is missing
...
awk '
  /#include "flutter\/generated_plugin_registrant.h"/ {
    print;
    print "#include <flutter_cef/flutter_cef_plugin.h>"
    next
  }
  { print }
' "$file" > "$file.tmp" && mv "$file.tmp" "$file"

echo "Added '#include <flutter_cef/flutter_cef_plugin.h>' after '#include \"flutter/generated_plugin_registrant.h\"'"
